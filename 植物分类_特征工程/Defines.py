...
# 默认path = 'all_data'  # 数据集位置
path = 'all_data'
is_preload = False # 是否读取预处理的数据 默认：False

SIFT_BOW_size = 200  # Sift中,词袋大小(降维后的特征数量)默认 80
HOG_PCA_size = 200  # HOG降维后的特征数量设置 默认60
LBP_PCA_size = 200  # LBP降维后的特征数量设置 默认80
...
def Make_CSV(predict_label, filename):
    ...
    file = pandas.DataFrame(columns=['ID', 'Category'], data=list(zip(files, predict_label)))
