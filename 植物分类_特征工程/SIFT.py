...
if __name__ == '__main__':
    ...
    # 训练分类器、进行预测、计算正确率，默认选择的分类器是SVC
    clf = Clf_SVC(train_X, train_Y)
    pred = clf.predict(val_X)
    accuracy(val_Y, pred)

    # 训练分类器、进行预测、计算正确率，此处选择随机森林
    # clf = Clf_RandomForestClassifier(train_X, train_Y)
    # pred = clf.predict(val_X)
    # accuracy(val_Y, pred)

    # ExtraTreesClassifier
    # clf = Clf_ExtraTreesClassifier(train_X, train_Y)
    # pred = clf.predict(val_X)
    # accuracy(val_Y, pred)

    # XGBooster
    # clf = Cls_XGBooster(train_X, train_Y)
    # pred = clf.predict(val_X)
    # accuracy(val_Y, pred)

    # KNN
    # clf = Clf_KNN(train_X, train_Y)
    # pred = clf.predict(val_X)
    # accuracy(val_Y, pred)