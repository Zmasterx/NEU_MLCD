...
def Image_Equalize(image):
    ...
    # cv2.imshow("Equalize", numpy.hstack([image, result]))
    # cv2.waitKey(0)
    return result
...
def Image_Sharpening(image):
    ...
    cv2.imshow("Sharpening", numpy.hstack([image, result]))
    cv2.waitKey(0)
    return result

def Image_Emboss(image):
    """对图像使用Emboss算子进行效果"""
    emboss_op = numpy.array([[-2, -1, 0], [-1, 1, 1], [0, 1, 2]], 'float32')
    result = cv2.filter2D(image, cv2.CV_32F, emboss_op)
    result = cv2.convertScaleAbs(result)
    cv2.imshow("Equalize", numpy.hstack([image, result]))
    cv2.waitKey(0)
    return result

def Image_SobelFeldman_Horizontal(image):
    """使用Sobel-Feldman算子进行水平边缘检测"""
    sobel_feldman_horizontal = numpy.array([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], 'float32')
    result = cv2.filter2D(image, cv2.CV_32F, sobel_feldman_horizontal)
    result = cv2.convertScaleAbs(result)
    cv2.imshow("Equalize", numpy.hstack([image, result]))
    cv2.waitKey(0)
    return result
...
def Image_Split(mask, image, is_display=False):
    ...
    if is_display:
        print(image.shape)
        cv2.imshow("Split", numpy.hstack([image, result]))
        cv2.waitKey(0)
        print('图片展示完毕')
        cv2.imwrite('text.png',result)
        exit()
    return result
...
def Process_All_StartUp():
    ...
    if number_limit == '0':
        ...
        result = Image_Sharpening(result)
        Image_Split(image_test, result, is_display=True)
    else:
...
image_test = cv2.imread('data/train/Charlock/aahijusizs.png')

#这部分代码可以进行示例
Process_All_StartUp()


#下面的代码展示了进行sift提取特征点之后的展示
# result = Image_Equalize(image_test)
# result = Image_Split(result,result,True)
# detector = cv2.SIFT_create()
# kp, sift = detector.detectAndCompute(result, None)
